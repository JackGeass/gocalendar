...
// 日历:公历和农历,节气，天干地支
func TestCalendar_CalendarsDetail(t *testing.T) {
	sc := Calendar{
		Loc:       time.Local, // 时区,默认time.Local
		FirstWeek: 0,          // 从周几天始,默认0周日开始, (日历表第列是周几,0周日,依次最大值6)
		Grid:      GridDay,    // 取日历方式,默认取一个月, gocalendar.GridDay取一天,gocalendar.GridWeek 取一周,gocalendar.GridMonth取一个月
		Zwz:       false,      // 是否区分早晚子时(子时从23-01时),true则23:00-24:00算成上一天
		Getjq:     true,       // 是否取节气
	}
	cds := sc.CalendarsDetail(2020, 2, 4, 17, 3, 44)
	for _, cd := range cds {
		var jqStr string
		if cd.SD.Jq != nil {
		...
		t.Logf("公历:%d年%d月%d日 周%s 农历:%d(%s%s)[%s]年%s%s月%s 干支%s %s", cd.SD.Date.Year, cd.SD.Date.Month, cd.SD.Date.Day, WeekChinese(cd.SD.Date.Week), cd.LD.Date.Year, cd.LD.YearGanZi.Gan, cd.LD.YearGanZi.Zhi, cd.LD.YearGanZi.Animal, cd.LD.LeapStr, cd.LD.MonthStr, cd.LD.DayStr, jqStr,
	...
	cds = sc.CalendarsDetail(2020, 2, 4, 17, 3, 45)
	for _, cd := range cds {
		var jqStr string
		if cd.SD.Jq != nil {
			jqStr = fmt.Sprintf(" 节气:%s (定%s：%d:%d:%d)", cd.SD.Jq.Name, cd.SD.Jq.Name, cd.SD.Jq.Date.Hour, cd.SD.Jq.Date.Min, cd.SD.Jq.Date.Sec)
	...
	cds = sc.CalendarsDetail(2020, 2, 4, 23, 00, 00)
	for _, cd := range cds {
		var jqStr string
		if cd.SD.Jq != nil {
			jqStr = fmt.Sprintf(" 节气:%s (定%s：%d:%d:%d)", cd.SD.Jq.Name, cd.SD.Jq.Name, cd.SD.Jq.Date.Hour, cd.SD.Jq.Date.Min, cd.SD.Jq.Date.Sec)
	...
	cds = sc.CalendarsDetail(2020, 2, 4, 23, 0, 1)
	for _, cd := range cds {
		var jqStr string
		if cd.SD.Jq != nil {
			jqStr = fmt.Sprintf(" 节气:%s (定%s：%d:%d:%d)", cd.SD.Jq.Name, cd.SD.Jq.Name, cd.SD.Jq.Date.Hour, cd.SD.Jq.Date.Min, cd.SD.Jq.Date.Sec)
